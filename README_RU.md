...
SSIM (структурное сходство изображений) сравнивает паттерны изменения интенсивности пикселей, которые являются важными атрибутами для человеческого зрения. Оценка SSIM варьируется от -1 до +1, где значение 1 указывает на идентичные изображения. Процесс можно разделить на три компонента:
